...
def generate_vehicle_json():
        ...
        for vehicle_name, vehicle_data in vehicles.items():
            ...
            if "affectations" in vehicle_data:
                ...
                for current_affectation in vehicle_data["affectations"]:
                    ...
                    vehicle_name_to_id[f"{vehicle_name}>{current_affectation}"] = (
                        last_id,
    ...
    with open(
        os.path.join(RESULT_DIR, "timeline_groups_dataset.json"), "w"
    ) as vehicles_dataset:
...
def generate_timeline_json(vehicle_id):
    ...
    with open(os.path.join(SOURCE_DIR, "timeline_source.json"), "r") as source_file:
        ...
        for duty_date in sorted(source_dict.keys()):
            ...
            if "affectations" not in duty_data:
                logger.error(
                    'Could not find an "affectations" section for the period %s',
                    duty_date,
                )
                continue
            ...
            for current_vehicle_name, current_vehicle_data in duty_data[
                "affectations"
            ].items():
                group, classname = vehicle_id[current_vehicle_name]
                if current_vehicle_data:
                    affectation_date_start, affectation_date_end = get_period(
                ...
                dataset.append(
                    {
                        "start": (
                            affectation_date_start.isoformat()
                            if current_vehicle_data
                            else date_start.isoformat()
                        ),
                        "end": (
                            affectation_date_end.isoformat()
                            if current_vehicle_data
            ...
            if last_end_date:
                ...
                logger.info(
                    "hidden dates : last_end_date %s, start %s",
                    last_end_date,
            ...
            logger.debug(
                "last_timeline_date : %s, date_end : %s", last_timeline_date, date_end
            )
    ...
    options_json = {
        ...
        "maxHeight": "95%",
        "nextStartDate": (
            last_timeline_date + datetime.timedelta(days=-60)
        ).isoformat(),
        "stack": False,
        "locale": "fr",
    ...
    with open(
        os.path.join(RESULT_DIR, "timeline_options.json"), "w"
    ) as hidden_dates_file:
    ...
    with open(
        os.path.join(RESULT_DIR, "timeline_dataset.json"), "w"
    ) as timeline_output_file:
        json.dump(dataset, timeline_output_file)


def get_inter(vehicle_id):
    with open(
        os.path.join(SOURCE_DIR, "interventions.csv"), "r", encoding="UTF-8"
    ) as interventions_file:
        for current_intervention in csv.DictReader(interventions_file):
            ...
            yield {
                ...
                **{
                    "title": "{} : {} - {}".format(
                        current_intervention["content"],
                        current_intervention["start"],
                        current_intervention["end"],
                    ),
                    "group": group,
                    "className": classname,
                },
...
def get_period(date_str: str) -> Tuple[datetime.datetime, datetime.datetime]:
    if ">" in date_str:
        ...
        return datetime.datetime.fromisoformat(start), datetime.datetime.fromisoformat(
            end
        )
